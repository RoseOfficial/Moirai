...
public static class DebugReport
{
    public static string Build(Plugin plugin, SnapshotBuilder snapshots)
    {
        ...
        sb.AppendLine($"canMount={snap?.Player.CanMount.ToString() ?? "?"} canFly={snap?.Player.CanFly.ToString() ?? "?"} flightStatus={GameEx.FlightStatus()} zoneFlightAllowed={!ZoneData.NoFlyTerritories.Contains((ushort)Svc.ClientState.TerritoryType)} mounted={snap?.Player.IsMounted.ToString() ?? "?"}");
        sb.AppendLine($"dodge={snap?.DodgeReady.ToString() ?? "?"} danger={snap?.Danger.ToString() ?? "?"} textAdvance={snap?.TextAdvanceReady.ToString() ?? "?"} inDuty={snap?.Player.InDuty.ToString() ?? "?"} dutyPopped={snap?.DutyPopped.ToString() ?? "?"}");
