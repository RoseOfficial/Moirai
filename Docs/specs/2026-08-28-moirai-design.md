...
- B15. Search: the game lists only what stands near the character, so in a wide ring every one of the FATE's enemies can be out of sight from the dropoff, and the engage behavior used to wait there for them until the FATE ended. With none in view, no stray target held (B10), and no boss fight in combat (B9), it heads for the ring center (within a quarter of the radius counts), looks from there for 3 s for a wave between spawns, then walks six waypoints on a circle at 0.6 of the radius, each resolved to the floor, starting opposite where the search began since that side has been seen, until an enemy shows up. A waypoint not got closer to within 3 s is given up for the next, so a center inside a rock does not hold the search. Every fight ends the search, and the next one starts again from the center. A collect FATE with nothing to pick up and nothing in view searches the same way.
