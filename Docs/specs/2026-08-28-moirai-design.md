...
- **Battle** (default): engage the FATE's boss when one stands among adds (B13), otherwise the nearest FATE enemy; sticky targeting (keep target until dead/invalid); melee/ranged stop distances by job category; never path into hitboxes; a walk is stopped once the target is in range, so the fight stands its ground (B16); while in combat, in-fight repositioning belongs to the dodge layer, not navigation. With none of the FATE's enemies in view (the game lists only what stands near the character) and the character out of combat, head for the ring center, look from there, then walk a circle inside the ring until one shows up (B15).
...
- **Ring knockback**: outside the ring with the FATE still running → path back in to just inside the edge on the character's side (C5).
...
vnavmesh for all pathing (`PathfindAndMoveCloseTo` with tolerance; floor/nearest-mesh queries for landable points). Mount when the leg exceeds a threshold and mounting is legal, asking for at most a bounded window before walking the leg, and asking again after a retry window while the leg is still long (C15); the ask drops the running path first, since a mount cast is interrupted by movement (C18); fly wherever the game says the mounted character may take off (C16) *and* the zone's data file permits (a per-zone no-fly override exists for zone geometry that breaks flight pathing, empty until a zone earns an entry); the mount asked for is the flying roulette where the character may take off, the plain roulette elsewhere, or a mount the player picked (C21); sprint on long walks on foot (C20). Landing targets are randomized points within the ring resolved to the mesh floor — never the raw center, which may be unlandable. Once the FATE's enemies come into view on the way in and none stands near the dropoff, it moves next to the one the fight will start on (C19). Altitude-ceiling errors during flight fall back to teleporting. A leg starts with a teleport when an attuned aetheryte's route beats the direct path by the teleport penalty (C17), the same cost model the A12 ranking uses; the teleport is held until the character stands at the aetheryte, or given up on after 20 s.
...
- B15. Search: the game lists only what stands near the character, so in a wide ring every one of the FATE's enemies can be out of sight from the dropoff, and the engage behavior used to wait there for them until the FATE ended. With none in view, no stray target held (B10), and the character out of combat, it heads for the ring center (within a quarter of the radius counts), looks from there for 3 s for a wave between spawns, then walks six waypoints on a circle at 0.6 of the radius, each resolved to the floor, starting opposite where the search began since that side has been seen, until an enemy shows up. A waypoint not got closer to within 3 s is given up for the next, so a center inside a rock does not hold the search. Every fight ends the search, and the next one starts again from the center. A collect FATE with nothing to pick up and nothing in view searches the same way. In combat the search waits where it stands, whatever the FATE's kind (B9 was the boss case of it): a gap between kills, an enemy gone untargetable for a moment, or the combat flag lingering after the last kill means the fight is here. Until then every such gap in a battle FATE sent the character to the center and around the ring while still in combat.
- B16. The fight stands its ground: a walk of the engage behavior (closing on the target, the search, the ring re-entry) is a navigation path that runs until it arrives or is stopped, whatever is asked for next, so the first standstill after one (fighting in range, a boss fight in combat, the in-combat wait) stops the path before anything else. The dodge layer's handover stops it too and counts. Until then the path ran on under the fight: an enemy that came into range during the search was fought while the character walked on to the next waypoint, which starts on the far side of the ring from the last fight, and a close walked on to where the target had stood when the path was issued, breaking casts on the way.
...
- C5. Knocked out of ring, FATE running → path back in to the point on the line from the center to the character 5 y inside the edge (half the radius at most, in a small ring), resolved to the floor and kept while the character stays out. Until then the path went to the center, up to the whole radius away from a fight at the edge, and a target standing past the edge had the character walk out to it and back to the center on every close.
...
- C15. Mounting is asked for at most 6 s in a row; then the leg is walked and the stall sampler starts fresh. A mount that took clears the budget, so a knock off the mount mid-leg mounts again. Without the budget a character who cannot mount stood still forever, the wait suppressing the sampler as a mount cast. A walked leg asks again after 20 s while the rest of it still exceeds the mount threshold, so a mount that failed once, at a no-mount spot or from an ask that landed mid-movement, is not a walk across the map.
- C16. Flight wherever the game says the mounted character may take off, the same check it runs when the mounted player jumps: the zone's currents or its unlock quest, and the mount's own flying condition. C1's overrides apply on top. The first cut reconstructed the rule from the sheets (the zone's aether-current set complete), which is not the game's rule: every ARR zone links to the one Mor Dhona set rather than to a set of its own, its flight coming from the 2.0 finale quest rather than from currents, and a mount can carry a flying condition of its own. An airborne path a ground mount cannot follow fed the ladder on every leg, and a ground path under a mount that could have flown rode the whole leg on the ground.
...
- C18. The mount ask drops the running path first and forgets it: a mount cast is interrupted by movement, so an ask that landed while vnavmesh was still moving the character (the leg's first GoTo goes out on foot while combat lingers after a FATE) never took, the C15 budget ran out, and the whole leg was walked. A GoTo whose fly flag changed is issued as a fresh path, since the same destination into a running path is a no-op (C13) and the ride stayed on the ground after the mount took.
- C19. Land where the fight is: once the FATE's enemies are in view on the way in and none stands within 15 y of the dropoff, the dropoff moves next to the one the fight will start on (the engage behavior's own pick, B7/B13 first, else the nearest to the player), 6 y short of its hitbox on the player's side, pulled in to the dropoff circle (C3) when the enemy stands past it, and resolved to the floor. Each enemy is aimed at once per leg, so a spot with no floor is asked about once; never once the landing has begun; escort and unopened FATEs keep the random point, since they are not a fight first. Until then the random point could fall on the far side of a wide ring from the whole fight.
- C20. Sprint: whichever behavior walks (a walked leg, the ring search, an approach to an NPC or an item), a walk on foot out of combat longer than 20 y asks for sprint, and the executor presses it only when the game reports it ready (general action 4: 20 s out of combat, 56 s recast). Never from the saddle, never in combat, where the recast is the rotation's to spend; a setting turns it off. The design always called for it and no code did it until then.
- C21. Mount choice: where the character may take off (C16, with C1's overrides and the flight setting), the ask is the Flying Mount Roulette (general action 24) whenever the game reports it ready, so the draw is never a mount whose own flying condition is not met, which C16 then rides on the ground; elsewhere the plain Mount Roulette (general action 9). A setting keeps the plain roulette everywhere, or names a mount by `Mount` row, used while this character owns it and falling back to the roulette otherwise. Executor-side: the planner only asks to mount.
